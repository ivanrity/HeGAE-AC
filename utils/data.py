...
def load_ACM_data_full(prefix='data/preprocessed/ACM_processed'):
    ...
    # features_0 = load_npz(prefix + '/features_0.npz').toarray()
    # features_1 = load_npz(prefix + '/features_1.npz').toarray()
    # features_2 = load_npz(prefix + '/features_2.npz').toarray()
    # features_list = [features_0,features_1,features_2]
    adjM = load_npz(prefix + '/adjM.npz').toarray()
    type_mask = np.load(prefix + '/node_types.npy')
    labels = np.load(prefix + '/labels.npy')
    train_val_test_idx = np.load(prefix + '/train_val_test_idx.npz')
    with open('RGCN/data/ACM_metapath_all.pkl','rb') as tf:
        data = pickle.load(tf)
        X = []
        for i in range(len(data['X'])):
            coo = data['X'][i].tocoo()
            feat_tensor = torch.sparse.FloatTensor(torch.LongTensor([coo.row.tolist(), coo.col.tolist()]),
                                                   torch.FloatTensor(coo.data.astype(np.float)))
            X.append(feat_tensor)
        adj_full = data['adj_truth']
...
def load_IMDB_data(prefix='data/preprocessed/IMDB_processed'):
    ...
    # features_0 = load_npz(prefix + '/features_0.npz')
    # features_1 = load_npz(prefix + '/features_1.npz')
    # features_2 = load_npz(prefix + '/features_2.npz')
    adjM = load_npz(prefix + '/adjM.npz')
    type_mask = np.load(prefix + '/node_types.npy')
    labels = np.load(prefix + '/labels.npy')
    train_val_test_idx = np.load(prefix + '/train_val_test_idx.npz')

    with open('RGCN/data/IMDB_metapath_free.pkl','rb') as tf:
        data = pickle.load(tf)
        X = []
        for i in range(len(data['X'])):
            coo = data['X'][i].tocoo()
            if i==0:
                coo.row = np.hstack([coo.row,np.array([4277])])
                coo.col = np.hstack([coo.col,np.array([3065])])
                coo.data = np.hstack([coo.data,np.array([0])])
            feat_tensor = torch.sparse.FloatTensor(torch.LongTensor([coo.row.tolist(), coo.col.tolist()]),
                                                   torch.FloatTensor(coo.data.astype(np.float)))
            X.append(feat_tensor)
        adj_full = data['adj_truth']
...
def load_DBLP_data(prefix='data/preprocessed/DBLP_processed'):
    ...
    labels = np.load(prefix + '/labels.npy')
    train_val_test_idx = np.load(prefix + '/train_val_test_idx.npz')
    with open('RGCN/data/DBLP_metapath_all.pkl','rb') as tf:
        data = pickle.load(tf)
        X = []
        for i in range(len(data['X'])):
            coo = data['X'][i].tocoo()
    ...
    return [adjlist00, adjlist01, adjlist02], \
           ...
           X,\
           adj_full
def load_Yelp_data(prefix='data/preprocessed/Yelp'):
    G00 = nx.read_adjlist(prefix + '/0/0-1-0.adjlist', create_using=nx.MultiDiGraph)
    G01 = nx.read_adjlist(prefix + '/0/0-2-0.adjlist', create_using=nx.MultiDiGraph)
    G02 = nx.read_adjlist(prefix + '/0/0-3-0.adjlist', create_using=nx.MultiDiGraph)

    idx00 = np.load(prefix + '/0/0-1-0_idx.npy')
    idx01 = np.load(prefix + '/0/0-2-0_idx.npy')
    idx02 = np.load(prefix + '/0/0-3-0_idx.npy')
    ...
    labels = np.load(prefix + '/labels.npy')
    train_val_test_idx = np.load(prefix + '/train_val_test_idx.npz')
    with open('RGCN/data/Yelp.pkl','rb') as tf:
        data = pickle.load(tf)
    X = []
    for i in range(len(data['X'])):
        coo = data['X'][i].tocoo()
        feat_tensor = torch.sparse.FloatTensor(torch.LongTensor([coo.row.tolist(), coo.col.tolist()]),
                                               torch.FloatTensor(coo.data.astype(np.float)))
        X.append(feat_tensor)
    adj_full = data['adj_truth'].astype(np.float32)
    return [[G00, G01,G02]], \
           [[idx00, idx01, idx02]], \
           adjM, \
           type_mask,\
...
def load_ACM_data(prefix='data/preprocessed/ACM_processed'):
    ...
    labels = np.load(prefix + '/labels.npy')
    train_val_test_idx = np.load(prefix + '/train_val_test_idx.npz')
    with open('RGCN/data/ACM_metapath_free.pkl','rb') as tf:
        data = pickle.load(tf)
        X = []
        for i in range(len(data['X'])):
            coo = data['X'][i].tocoo()
...
def load_Yelp_data_mb(prefix='data/preprocessed/Yelp'):
    in_file = open(prefix + '/0/0-1-0.adjlist', 'r')
    adjlist00 = [line.strip() for line in in_file]
    adjlist00 = adjlist00[3:]
    in_file.close()
