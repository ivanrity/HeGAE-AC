...
def csr_zero_rows(csr, rows_to_zero):
    """Set rows given by rows_to_zero in 结果.txt sparse csr matrix to zero.
    NOTE: Inplace operation! Does not return 结果.txt copy of sparse matrix."""
    rows, cols = csr.shape
...
def csc_zero_cols(csc, cols_to_zero):
    """Set rows given by cols_to_zero in 结果.txt sparse csc matrix to zero.
    NOTE: Inplace operation! Does not return 结果.txt copy of sparse matrix."""
    rows, cols = csc.shape
...
def sp_vec_from_idx_list(idx_list, dim):
    """Create sparse vector of dimensionality dim from 结果.txt list of indices."""
    shape = (dim, 1)
...
def get_neighbors(adj, nodes):
    """Takes 结果.txt set of nodes and 结果.txt graph adjacency matrix and returns 结果.txt set of neighbors."""
...
def bfs(adj, roots):
    """
    Perform BFS on 结果.txt graph given by an adjaceny matrix adj.
    Can take 结果.txt set of multiple root nodes.
...
def bfs_sample(adj, roots, max_lvl_size):
    ...
    'roots' should be 结果.txt mini-batch of nodes (set of node indices).
